...
class BackgroundMovement:
    def __init__(self, fenetre):
        ...
        self.boutton_commencer = customtkinter.CTkButton(fenetre, text="PLAY", **self.Button_Style,
                                                         command=self.Play)  # bouton si pour rejouer et appelle au fonction game
        self.boutton_commencer.place(relx=0.45, rely=0.5)
    ...
    def Play(self):
        self.start_movement()
        self.boutton_commencer.destroy()

    def game_over(self):
        self.image = Image.open("../Image/game_over.jpg")  # on prend l'image dans son emplacement
        self.image = self.image.resize((600, 600))  # on change sa forme si nécessaire
        self.image = ImageTk.PhotoImage(self.image)  # on déclare l'image dans un variable
        self.fond = self.canvas.create_image(300, 200, anchor=tk.CENTER,
                                             image=self.image)  # création de l'image et definit sa position
        self.boutton_rejouer = customtkinter.CTkButton(fenetre, text="PLAY AGAIN", **self.Button_Style,
                                                         command=self.car)  # bouton si pour rejouer et appelle au fonction game
        self.boutton_rejouer.place(relx=0.45, rely=0.8)
        self.canvas.delete(self.car_explosion)
        self.score=self.score

    def car(self):  # fonction si on veut jouer encore
        self.boutton_rejouer.destroy()

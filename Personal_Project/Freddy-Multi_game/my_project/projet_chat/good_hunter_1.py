...
class fish(object):
    def __init__(self,can):
        ...
        self.chat = self.canvas.create_image(self.x, self.y, image=self.image_chat1)
        ...
        self.chat_user = self.canvas.create_image(self.x1, self.y1, image=self.image_chat)
    ...
    def rat_0(self): #fonction pour le faire apparaître
        ...
        self.rat = self.canvas.create_image(self.x_rat +50, self.y_rat, image=self.image_rat)
        self.rat_mouve()
    ...
    def rat_mouve(self): #fonction pour la mouvement de souris
        ...
        if (self.x) >= (self.x_rat) and (self.x - 100) <= (
                self.x_rat):  # condition pour avoir de score et pour quitter le jeu si le score max est atteint
            if (self.y) >= (self.y_rat) and (self.y - 100) <= (self.y_rat):
                ...
                if self.score >= 99:
                    ...
                    window.destroy()
        self.canvas.after(self.dt, self.rat_mouve)

    def mouve_chat(self):
        self.canvas.moveto(self.chat, self.x, self.y)  # on donne le mouvement du souris
        self.x -= (self.deltax*10) # on diminue la valeur de z suivant
        self.y -= (self.deltay*10) # on diminue la valeur de x
        if self.x <= 50:
            self.x = 50
        if self.x >= 500:
            self.x = 500
        ...
        if self.y >= 500:
            self.y = 500
        if not 130 < self.y < 500: #condition pour la mouvemnent de souris, pour qu'il revient
            self.deltay = -self.deltay
            self.x_rat = -self.deltax

        if not 50 < self.x < 550:
            self.deltax = -self.deltax


        self.canvas.after(1000, self.mouve_chat)  # appel au fonction pour qu'il continue le mouvement

    def chat_1(self, event):  # mouvement de la tank
        if event.keysym == "Left":
            self.x1 -= self.rect_mouv_x
            if self.x1 < 100:
                self.x1 = 100
        ...
        canvas.coords(self.chat_user, self.x1-25, self.y1-25)

        if event.keysym == "Up":
            self.y1 -= self.rect_mouv_y
            if self.y1 < 130:
                self.y1 = 130
...
if __name__ == '__main__':
    ...
    #insertion de photo
    image = tk.PhotoImage(file="../Image/fish.PNG")
    image = image.zoom(3) #zoom por l'arrière plan

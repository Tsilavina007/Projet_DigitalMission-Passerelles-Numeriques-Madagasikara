...
class Tank:
    ...
    def rotate_stick(self, event):   #fonction appelé lorsqu'il y a un événement de rotation de la molette de la souris., elle permet de faire l'inclinaison du canon
    ...
    def move_tank(self, event):  # fonction appelé  lorsqu'il y a un événement de déplacement de la souris. Elle permet de déplacer le tank horizontalement sur le canevas.
    ...
    def shoot(self, event):
        ...
        pygame.init()
        sound1 = pygame.mixer.Sound("song/canoon+4.mp3")  # Charger le son du canon
        ...
        boom = tk.PhotoImage(file="img/blast.png")   # Charge l'image de l'explosion.
        ...
        while ball_y <= 450 and ball_x <= 800 and self.is_shooting == True:    # Boucle tant que la balle est dans les limites du canevas et que le tir est en cours.
            ...
            if (self.x < ball_x and (self.x + 50) > ball_x) and (self.y + 15 < ball_y and (self.y + 45) > ball_y):
                ...
                sound3 = pygame.mixer.Sound("song/Explosion+7.mp3")  # Charger le son de l'explosion
...
# Pendant la Déplacement automatique de l’avion de gauche à droite.
    # le tank Effectue une action de "repost" dans le jeu.
    # la  deplacement de la balle de la repost de l'avion de manière progressive vers une position cible.
    # Affichage d'une jauge de vie du tank.
    # Réduction de la jauge de vie si la balle de l'avion atteint le tank.
    # Affichage d'un message de "Game Over" si la jauge de vie atteint le seuil de défaite.
    # Possibilité de rejouer en appuyant sur un bouton de réinitialisation.
    ...
    def rpst(self):   # effectuer une action de "repost" dans le jeu.
        ...
        if self.x < new_tank_x:
            image02 = tk.PhotoImage(file="img/torpedo1.png")  # Charge l'image du repost
            ...
            image02 = tk.PhotoImage(file="img/torpedo.png")  # Charge l'image du repost
            image_repost = image02.subsample(25, 25)  # Redimensionne l'image
            bal_repost = self.canvas.create_image(self.x, self.y,
                                                  image=image_repost)  # Crée l'image du repost sur le canevas
...
#Anime le repost du tank en le déplaçant de manière progressive vers une position cible.
    def repost(self):
        ...
        if self.yr < 440:  # Vérifie si le repost est encore dans les limites du canevas
            ...
            if (self.yr == 400 and self.xr > new_tank_x - 75 and self.xr < new_tank_x + 75) or \
                ...
                if self.vie2 < 400 and self.vie2>351:
                    ...
                    sound4 = pygame.mixer.Sound("song/song4.mp3")  # Charger le son de l'explosion
                    ...
                    image03 = tk.PhotoImage(file="img/blasting.png")  # Charge l'image de l'explosion du repost
                    ...
                    sound4 = pygame.mixer.Sound("song/gameover.mp3")  # Charger le son de l'explosion
    ...
    def mvm(self):
        ...
        self.x += 5  # Déplace l'avion horizontalement

        if self.x in [240, 440, 640]:
            self.rpst()  # Déclenche le repost si le ballon atteint certaines positions

        self.canvas.moveto(ballon, self.x, self.y)  # Déplace l'image (l'avion) à la nouvelle position
        ...
        if self.x >= 800:
            self.canvas.delete(ballon)  # Supprime l'avion
            self.create_balloon()  # Crée un nouveau avion
            return  # Termine la méthode
        else:
            self.id1 = self.canvas.after(70, self.mvm)  # Planifie le prochain mouvement de l'avion

    def create_balloon(self): # creation de l'avion
        ...
        # Charger l'image
        image01 = tk.PhotoImage(file="img/rocket.png")
        ...
        self.mvm()     # appel la methode mvm pour la mouvement de l'avion
...
if __name__ == "__main__":
    ...
    pygame.mixer.music.load("song/song2.mp3")  # Charge le fichier audio "song2.mp3"
    ...
    image_fond = Image.open("img/fond.jpg")

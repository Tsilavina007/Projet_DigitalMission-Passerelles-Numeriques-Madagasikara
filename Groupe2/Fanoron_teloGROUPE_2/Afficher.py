...
selected_pion = None  # Variable pour stocker la position du pion sélectionné par le joueur pour le déplacement
...
def on_canvas_click(event):
    global current_player, total_moves, selected_pion
    ...
    if current_player == "player":
        ...
        if 75 <= x <= 225 and 75 <= y <= 225:
            ...
            if selected_pion is None:
                # Vérifier si la case est un pion du joueur
                if (x, y) in player_pions:
                    selected_pion = (x, y)
                    canvas.create_text(x * 75, y * 75, text="X", fill="blue", font=("Helvetica", 20, "bold"))

            else:
                # Vérifier si la case est libre avant de déplacer le pion du joueur
                if (x, y) not in player_pions and (x, y) not in computer_pions:
                    ...
                    player_pions.remove(selected_pion)
                    ...
                    # Annuler la sélection du pion si la case n'est pas libre
                    selected_pion = None

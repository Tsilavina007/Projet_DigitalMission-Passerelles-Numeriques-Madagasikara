...
from tkinter import ttk   # Sous module pour le trackbar


# Classe principale pour le lecteur de musique
class Player(tk.Frame):
    def __init__(self, master):
        super().__init__(master, bg="teal")
        self.master = master
        self.pack()
        ...
        self.background_images = [  # Liste des images de fond
            "Images/Fond1.gif",
            "Images/Fond2.gif",
            "Images/Fond3.gif",
            "Images/Fond4.gif",
            ...
            "Images/Fond6.gif",
            "Images/Fond8.gif",
            "Images/Walpaper9.gif",
            "Images/Zepeto Dancing GIF - Zepeto Dancing Dancer - Discover & Share GIFs.gif",
            "Images/Mus.gif",
            "Images/g.gif",
            "Images/Sary.gif",
        ]
    ...
    def create_frames(self):
        ...
        self.tracklist.config(width=1000, height=1000)
        self.tracklist.grid(row=1, column=2, rowspan=2, pady=20)
    ...
    def track_widgets(self):
        ...
        # Ajouter une barre de progression pour montrer la progression des chansons en cours de lecture
        self.progressbar = ttk.Progressbar(
            self.track, orient=tk.HORIZONTAL, length=500, mode="determinate"
        )
        self.progressbar.grid(row=2, column=0, pady=10)

    def control_widgets(self):
        # Crée un bouton pour charger des chansons
        self.loadSongs = tk.Button(self.controls, bg="white", fg="black", font=5)
    ...
    def retrieve_songs(self):  # fonction qui fait relie celle-ci avec l'ordinateur
    ...
    def play_song(self, event=None):  # Joue la chanson sélectionnée
        ...
        # Remplace self.change_background("Images/Fond1.gif") par ceci pour choisir une image aléatoire :
        self.change_background(random.choice(self.background_images))

        # Démarre la barre de progression
        self.start_progressbar()

    def start_progressbar(self):
        # Détermine la durée totale de la chanson en millisecondes
        total_duration = mixer.Sound(self.playerlist[self.current]).get_length() * 1000

        # Met à jour la barre de progression toutes les 100 millisecondes
        self.progressbar["maximum"] = total_duration
        self.update_progressbar()

    def update_progressbar(self):
        # Récupère le temps écoulé de la chanson en cours en millisecondes
        current_time = mixer.music.get_pos()

        # Met à jour la barre de progression avec le temps écoulé
        self.progressbar["value"] = current_time

        # Continue à mettre à jour la barre de progression toutes les 100 millisecondes jusqu'à la fin de la chanson
        if current_time < self.progressbar["maximum"]:
            self.after(100, self.update_progressbar)

    def change_background(self, image_path):
        self.img = PhotoImage(
            file=image_path
        )  # Utilisez self.img pour conserver une référence à l'image
    ...
    def prev_song(self):  # Passer à la chanson précédente dans la liste
        if self.current > 0:
            self.current -= 1
    ...
    def next_song(self):  # Passer à la chanson suivante dans la liste
...
img = PhotoImage(file="Images/Walpaper9.gif")
img = PhotoImage(
    file="Images/Zepeto Dancing GIF - Zepeto Dancing Dancer - Discover & Share GIFs.gif"
)
img = PhotoImage(file="Images/Sary.gif")
img = PhotoImage(file="Images/g.gif")
img = PhotoImage(file="Images/Fond2.gif")
img = PhotoImage(file="Images/Fond3.gif")
img = PhotoImage(file="Images/Fond4.gif")
img = PhotoImage(file="Images/Fond5.gif")
img = PhotoImage(file="Images/Fond6.gif")
img = PhotoImage(file="Images/Fond8.gif")
